...
if($id==999999){
	...
	$consulta  = "select id, lote, fiber_id, remaining_inches from rols where state_id in (3,2) and deleted_at is null order by id_fibra";	
	//echo"$consulta";
	...
	$dato="<select multiple='multiple' class='form-control' name='roll_id' id='fibroll_idra_id' size='10'><optgroup label='Rollo'>";
	while(@mysql_num_rows($resultado)>=$count)
	{		
	
		$res=mysql_fetch_row($resultado);
		$dato=$dato."<option value='$res[0]'>$res[2] / $res[1] - ($res[3] in)</option>";
		$count++;		
