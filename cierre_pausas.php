<?
    ...
    function lLog($var)
    {
        ...
        if(file_exists($nombre_archivo))
        {
			...
			$query = "INSERT INTO trace(valor)VALUES('$mensaje')";
			$exec_query = mysql_query($query) or die("Falla funcion trace: $query".mysql_error());
        }
        else
        {
    ...
    function trace($var)
	{
		$query = "INSERT INTO trace(valor)VALUES('$var')";
		$exec_query = mysql_query($query) or die("Falla funcion trace: $query".mysql_error());
	}
	...
	{ 
		...
		{		
			...
			echo $consulta;
			lLog($consulta );
			$resultado = mysql_query($consulta) or die("La consulta $consulta;P1: " . mysql_error());
			if (mysql_error()) {
				$falla = mysql_error();
				lLog("Error en insert cierre_turno: $resultado " . mysql_error() );
			}
	...
	{ 
		...
		{		
			...
			$consulta  =  "insert into cierre_turnos(turno,fecha, tipo,tiempo, location_id, grupo) values(".$turno_fin.", '".$fecha."',  99,".$activo.",".$res4['location_id'].",0)";
			lLog($consulta );
			echo $consulta;//$resultado = mysql_query($consulta) or die("Error en operacion1: $consulta " . mysql_error());
			$resultado = mysql_query($consulta);
			if (mysql_error()) {
				$falla = mysql_error();
				lLog("Error en insert cierre_turno: $resultado " . mysql_error() );
				echo "Error en operacion1: $consulta " . mysql_error();
			}
			//lLog($resultado );
		}
	}
	...
	{ 
		...
		{		
			$activo=540-$res4['d2'];
			$consulta  =  "insert into cierre_turnos(turno,fecha, tipo,tiempo, location_id, grupo) values(".$turno_fin.", '".$fecha2."',  99,".$activo.",".$res4['location_id'].",0)";
