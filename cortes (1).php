	...
	while(@mysql_num_rows($resultado)>=$count)
	{
		...
		?>
                            ...
                            <div class="col-sm-6 col-md-4 col-lg-2" id="d_pos">
                              <!--  <label for="location_id">Posición:</label>
                               <select class="form-control" id="number_position" name="number_position" onChange="mostrar(this.value);">
                                  <option value="0" selected="selected">--Selecciona--</option>
								   <?	  
	/*$consulta  = "SELECT number FROM location_capacities where deleted_at is null group  by number";
	$resultado = mysql_query($consulta) or die("La consulta fall&oacute;P1: " . mysql_error());
	$count=1;
	while(@mysql_num_rows($resultado)>=$count)
	{
