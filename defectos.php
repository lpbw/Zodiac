...
if($guardar=="Guardar")  
{
	...
	if($id!="")
	{
	$consulta  =  "update defects set nombre='$name' where id=$id";
	}
	else
	$consulta  =  "insert into defects(nombre) values('$name')";
	
	//out.println(listGStr);
	$resultado = mysql_query($consulta) or die("Error en operacion1: $consulta " . mysql_error());
	
	if($id!="")
	echo"<script>alert(\"Defecto Cambiado\");</script>";
	else
	echo"<script>alert(\"Defecto Agregado\");</script>";
...
if($borrar!="")  
{
	...
	$consulta  =  "delete from defects  where id=$borrar";
	//out.println(listGStr);
	$resultado = mysql_query($consulta) or die("Error en operacion1: $consulta " . mysql_error());
	
	
	echo"<script>alert(\"Defecto Borrada\");</script>";
	
	/*echo"<script>parent.tb_remove(); parent.location=\"adm_planes.php\"</script>";*/
}	
if($id!="")
{
	$consulta  = "SELECT *  FROM defects where id='".$_GET['id']."' ";
...
function borrar(id)
{
  if(confirm("¿Esta seguro de borrar este defecto?"))
  {
...
<nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">
    ...
    <div id="page-wrapper">
        ...
        <div class="col-lg-12 table-responsive">
                    ...
                    <tbody>
<?	  
	$consulta  = "SELECT id, nombre FROM defects  order by nombre";
	$resultado = mysql_query($consulta) or die("La consulta fall&oacute;P1: " . mysql_error());
	$count=1;
	while(@mysql_num_rows($resultado)>=$count)
	{
		...
		?>
		 <tr>
			...
			<td class="text-center"><a href="defectos.php?id=<? echo"$res[0]"; ?>"><i class="fa fa-pencil-square-o"></i></a></td>
