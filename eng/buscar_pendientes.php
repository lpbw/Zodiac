...
?>
 ...
 <table class="table table-striped table-condensed grid">
	...
	$consulta="SELECT cuts.id,mo, cn, cut_type.nombre, length_measured, locations.name, location_capacities.number, rolls.lote, roll_fibers.fiber_type, rolls.remaining_inches, cuts.status, cuts.parte  
		...
		?>
							<tr>
                                ...
                                <!--    <td>1</td>  ESTA ES LA LINEA VIEJA QUE REEMPLAZAMOS   -->
                                    ...
                                    <td> <? if($contado==0){?><input name="roll_id" type="hidden" id="roll_id" value="<? echo $res['id'];?>" /><select class="form-control" id="rollo_asignar" name="rollo_asignar" > <option value="0" selected="selected">--Choose--</option>
                                  <?	  
	$consulta2  = "SELECT number, fiber_id, lote, rolls.id FROM `location_capacities` left outer join rolls on location_capacities.id=rolls.location_slot where id_location =$locationU and location_slot<>0  and state_id=3 and rolls.fiber_id='".$res['fiber_type']."' order by number";
	...
	while(@mysql_num_rows($resultado2)>=$count2)
	{
		...
		echo"<option value=\"$res2[3]\" >$res2[0].- $res2[1] ($res2[2])</option>";
		$count2=$count2+1;
	}	
