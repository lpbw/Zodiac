...
<head>
    ...
    <title>Zodiac - Proveedores</title>
...
if($guardar=="Guardar")  
{
	...
	if($id!="")
	$consulta  =  "insert into roll_providers(name, created_at) values('$proveedor', now())";
	else
	$consulta  =  "update roll_providers set name='$proveedor', updated_at=now() where id=$id";
	//out.println(listGStr);
	$resultado = mysql_query($consulta) or die("Error en operacion1: $consulta " . mysql_error());
	
	if($id!="")
	echo"<script>alert(\"Proveedor Cambiado\");</script>";
	else
...
if($borrar!="")  
{
	...
	$consulta  =  "update roll_providers set deleted_at=now() where id=$borrar";
	//out.println(listGStr);
	$resultado = mysql_query($consulta) or die("Error en operacion1: $consulta " . mysql_error());
...
if($id!="")
{
	$consulta  = "SELECT *  FROM roll_providers where id='".$_GET['id']."' ";
...
function borrar(id)
{
  if(confirm("¿Esta seguro de borrar este proveedor?"))
  {
...
<nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">
    ...
    <div id="page-wrapper">
                ...
                <form action="" method="post" name="form1" class="margin-bottom" id="form1" accept-charset="utf-8">                                                
                  <div class="form-group">
						...
						<div class="col-sm-6 col-md-4 col-lg-4">
                                <label for="remaining_inches">Proveedor:</label>
        ...
        <div class="col-lg-12 table-responsive">
                    ...
                    <tbody>
<?	  
	$consulta  = "SELECT id, name FROM roll_providers where deleted_at is null order by name";
	$resultado = mysql_query($consulta) or die("La consulta fall&oacute;P1: " . mysql_error());
	$count=1;
	while(@mysql_num_rows($resultado)>=$count)
	{
		...
		?>
		 <tr>
			<td><? echo"$res[1]"; ?></td>
			<td class="text-center"><a href="proveedores.php?id=<? echo"$res[0]"; ?>"><i class="fa fa-pencil-square-o"></i></a></td>
    ...
    <script>
		...
		<? if($_GET['id']!=""){?>
	document.form1.proveedor.value='<? echo"$res8[1]";?>';
